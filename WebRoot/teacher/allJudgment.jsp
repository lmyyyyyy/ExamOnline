...
tr:last-child td:last-child {   border-radius: 0 0 9px 0;   }
  ...
  <body>
    <div>
    	<table>
    	<tbody>
    	<s:if test="judgmentList.isEmpty() == true">目前没有任何填空题!</s:if >
		
		<s:else>
			<table>
				<tr>
					<th style="width:20%" class="tdListHeader">题号</th>
					<th style="width:20%"class="tdListHeader">问题</th>
				...
				<s:iterator value="judgmentList">
					...
					<tr>
						...
						<td><s:a action="JudgmentAction_view?sid=%{#sId}" namespace="/" cssClass="aList">查看</s:a></td>
						<td><s:a action="JudgmentAction_modify?sid=%{#sId}" namespace="/" cssClass="aList">修改</s:a></td>
						<td><s:a action="JudgmentAction_delete?sid=%{#sId}" namespace="/" cssClass="aList">删除</s:a></td>
					</tr>
				</s:iterator>
    ...
    <tr sytle="width:200px">
					...
					<td align="center">
						<s:if test="#request.page.hasPrePage">
							<a href="JudgmentAction_allJudgment.action?currentPage=1">首页</a>
							<a href="JudgmentAction_allJudgment.action?currentPage=${page.currentPage -1 }">上一页</a>
						</s:if>
						<s:else>
							首页
							上一页
						...
						<s:if test="#request.page.hasNextPage">
							<a href="JudgmentAction_allJudgment.action?currentPage=${page.currentPage + 1 }">下一页</a>
							<a href="JudgmentAction_allJudgment.action?currentPage=${page.totalPage }">尾页</a>			
						</s:if>
						<s:else>
							下一页
							尾页
