		...
		<s:else>
			<table>
				<tr>
					...
					<th class="tdListHeader">打开/关闭</th>
					<th class="tdListHeader">查看</th>
					<th class="tdListHeader">收集成绩</th>
				...
				<s:iterator value="paperList">
					...
					<tr>
						...
						<td>
						<s:if test='status == 0'><s:a action="PaperAction_changeStatus?sid=%{#sId}" namespace="/" cssClass="aList">打开</s:a></s:if>
							<s:else><s:a action="PaperAction_changeStatus?sid=%{#sId}" namespace="/" cssClass="aList">关闭</s:a></s:else>
						</td>
						<td><s:a action="PaperAction_view?sid=%{#sId}" namespace="/" cssClass="aList">查看</s:a></td>
						<td><s:a action="CollectResultsAction?sid=%{#sId}" namespace="/" cssClass="aList">导出</s:a></td>
						<td><s:a action="PaperAction_delete?sid=%{#sId}" namespace="/" cssClass="aList">删除</s:a></td>
					</tr>
				</s:iterator>
			</table>
		</s:else>
