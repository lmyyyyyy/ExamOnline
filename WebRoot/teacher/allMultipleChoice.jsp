...
tr:last-child td:last-child {   border-radius: 0 0 9px 0;   }
  ...
  <body>
    <div>
    	<table>
    	<tbody>
    	<s:if test="multipleChoiceList.isEmpty() == true">目前没有任何多选题!</s:if >
		
		<s:else>
			<table>
				<tr>
				<th style="width:20%"class="tdListHeader">题号</th>
					<th style="width:20%"class="tdListHeader">多选问题</th>
					<!-- <td class="tdListHeader">选项A</td>
					<td class="tdListHeader">选项B</td>
					<td class="tdListHeader">选项C</td>
					<td class="tdListHeader">选项D</td>
					<td class="tdListHeader">选项E</td>
					<th class="tdListHeader">选项F</td> -->
					<th style="width:7%"class="tdListHeader">答案</th>
					<th style="width:7%"class="tdListHeader">科目</th>
					<th style="width:7%"class="tdListHeader">作者</th>
					<th style="width:12%"class="tdListHeader">创建时间</th>
				...
				<s:iterator value="multipleChoiceList">
					...
					<tr>
						...
						<td><s:property value="choicee" /></td>
						<td><s:property value="choicef" /></td> --%>
						...
						<td><s:a action="MultipleChoiceAction_view?sid=%{#sId}" namespace="/" cssClass="aList">查看</s:a></td>
						<td><s:a action="MultipleChoiceAction_modify?sid=%{#sId}" namespace="/" cssClass="aList">修改</s:a></td>
						<td><s:a action="MultipleChoiceAction_delete?sid=%{#sId}" namespace="/" cssClass="aList">删除</s:a></td>
					</tr>
				</s:iterator>
    ...
    <tr sytle="width:200px">
					...
					<td align="center">
						<s:if test="#request.page.hasPrePage">
							<a href="MultipleChoiceAction_allMultipleChoice.action?currentPage=1">首页</a>
							<a href="MultipleChoiceAction_allMultipleChoice.action?currentPage=${page.currentPage -1 }">上一页</a>
						</s:if>
						<s:else>
							首页
							上一页
						...
						<s:if test="#request.page.hasNextPage">
							<a href="MultipleChoiceAction_allMultipleChoice.action?currentPage=${page.currentPage + 1 }">下一页</a>
							<a href="MultipleChoiceAction_allMultipleChoice.action?currentPage=${page.totalPage }">尾页</a>			
						</s:if>
						<s:else>
							下一页
							尾页
