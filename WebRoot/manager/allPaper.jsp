			...
			<table>
				...
				<s:iterator value="paperList">
					...
					<tr>
						...
						<td>
						<s:if test='status == 0'><s:a action="PaperAction_mchangeStatus?sid=%{#sId}" namespace="/" cssClass="aList">打开</s:a></s:if>
								<s:else><s:a action="PaperAction_mchangeStatus?sid=%{#sId}" namespace="/" cssClass="aList">关闭</s:a></s:else>
						</td>
						<td><s:a action="PaperAction_view?sid=%{#sId}" namespace="/" cssClass="aList">查看</s:a></td>
						<td><s:a action="PaperAction_mdelete?sid=%{#sId}" namespace="/" cssClass="aList">删除</s:a></td>
					</tr>
				</s:iterator>
